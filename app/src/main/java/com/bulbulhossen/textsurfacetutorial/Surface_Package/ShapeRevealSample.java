...
public class ShapeRevealSample {
	public static void play(TextSurface textSurface) {
		...
		Text textA = TextBuilder.create("Bangladeshi News Portal").setPosition(Align.SURFACE_CENTER).build();
		Text textB = TextBuilder.create("247 News update ").setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textA).build();
		Text textC = TextBuilder.create("You want to fight, you come tonight.").setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textB).build();
		Text textD = TextBuilder.create("Developer by bangladeshi").setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textC).build();
