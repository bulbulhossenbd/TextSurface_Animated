...
public class SlideSample {
	public static void play(TextSurface textSurface, AssetManager assets) {
		...
		Text textA = TextBuilder.create("Bangladeshi").build();
		Text textB = TextBuilder.create("Developer").setPosition(Align.LEFT_OF, textA).build();
		Text textC = TextBuilder.create("WE ARE PROUD FILL").setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textB).build();
