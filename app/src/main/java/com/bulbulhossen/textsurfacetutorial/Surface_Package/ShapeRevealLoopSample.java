...
public class ShapeRevealLoopSample {
	public static void play(TextSurface textSurface, AssetManager assets) {
		...
		Text textA = TextBuilder.create("News Portal").setPosition(Align.SURFACE_CENTER).build();
		Text textB = TextBuilder.create("247 New Broadcasting").setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textA).build();
		Text textC = TextBuilder.create("You want to fight, you come tonight.").setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textB).build();
		Text textD = TextBuilder.create("Bangaldesh").setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textC).build();
